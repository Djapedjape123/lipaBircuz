...
function NavBar() {
    ...
    const navLinkStyle = ({ isActive }) =>
        `text-sm font-bold uppercase tracking-wider transition-colors duration-300 flex items-center gap-1 ${isActive ? 'text-emerald-400' : 'text-white hover:text-emerald-400'
        }`
    ...
    return (
        <>
            {/* 1. HEADER */}
            <header
                className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${shouldBeDark
            ...
            <div
                className={`lg:hidden fixed inset-0 z-40 transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'
